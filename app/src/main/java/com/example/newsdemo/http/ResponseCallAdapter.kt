        ...
        return flow {
            emit(
                suspendCancellableCoroutine { continuation ->
                    request(call, continuation)
                }
            )
        }
    }

    private fun request(call: Call<T>, continuation: CancellableContinuation<Response<T>>) {
...
class BodyCallAdapter<T>(private val responseType: Type) : CallAdapter<T, Flow<T>> {
    ...
    private fun request(call: Call<T>, continuation: CancellableContinuation<T>) {
        call.enqueue(object : Callback<T> {
            override fun onFailure(call: Call<T>, t: Throwable) {
                continuation.resumeWithException(t)
            }
